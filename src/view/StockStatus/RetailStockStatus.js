...
import { getStockStatusData } from '../../controller/response';
import { Button, Col, Drawer, Input, Row, Select, Table, DatePicker, Tag } from 'antd';
import { columnStockStatus } from '../../columns/column';
import { FilterOutlined, DownloadOutlined, SearchOutlined } from '@ant-design/icons';
import { getMarketData } from '../../controller/market';
import Dropdown from '../../components/Dropdown';
import { getCountryData } from '../../controller/country';
import { getPartnerTypeData } from '../../controller/partnerType';
import { getCategoryData } from '../../controller/category';
import { getSubCategoryData } from '../../controller/subcategory';
import { getPartnersData } from '../../controller/partner';
...
function RetailStockStatus() {
    ...
    const fetchData = async () => {
        ...
        const response = await getStockStatusData(categoryType, market, country, partnerType, partner, category, subcategory, sku, startDate, endDate, current, pageSize);
    ...
    const fetchMarket = async () => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getMarketData(category);
    ...
    const fetchCountry = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getCountryData(category, value);
    ...
    const fetchPartnerType = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getPartnerTypeData(category, market, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchCategory = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getCategoryData(category, market, country, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchSubcategory = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getSubCategoryData(category, market, country, partnerType, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchPartner = async (value) => {
        let dataPacket = [];
        let category = 'Retail';
        const response = await getPartnersData(category, market, country, value);
        //console.log(response.data)
        dataPacket = [...response.data];
        ...
        <div>
            <div style={{ border: '0.5px solid #f0f0f0', padding: '1em', borderRadius: '5px', background: '#f0f0f0' }}>
                <Row gutter={18}>
                    ...
                    <Col xs={24} sm={24} xl={8} xxl={6} md={12} lg={8} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Tag color='processing' style={{ padding: '0.2em 1em' }}><span style={{ fontSize: '1.2em', color: '#0095d9', fontWeight: '500' }}>Availablity Data found : </span><span style={{ fontSize: '1.2em', color: '#0095d9', fontWeight: '500' }}>{count}</span></Tag>
...
export default RetailStockStatus;