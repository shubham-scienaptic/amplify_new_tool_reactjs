...
import { getDigitalSelfData } from '../../controller/response';
import { Table, Row, Col, Button, Input, Tag, DatePicker } from 'antd';
import { FilterOutlined, DownloadOutlined, SearchOutlined } from '@ant-design/icons';
import { columnDigitalSelf } from '../../columns/column';
import Dropdown from '../../components/Dropdown';
import moment from 'moment';
import { getMarketDataShareOfSite } from '../../controller/market';
import { getCountryDataShareOfSite } from '../../controller/country';
import { getPartnerTypeDataShareOfSite } from '../../controller/partnerType';
import Loader from '../../components/Loader/Loader';
import { getPartnersDataShareOfSite } from '../../controller/partner';
import { getCategoryDataShareofSite } from '../../controller/category';
import { getSubCategoryDataShareOfSite } from '../../controller/subcategory';

const { RangePicker } = DatePicker;


function RetailDigitalSelf() {
    ...
    const fetchData = async () => {
        ...
        setLoader(true);
        let dataPacket = [];
        let categoryType = 'Retail'
        const response = await getDigitalSelfData(categoryType, market, country, partnerType, partner, category, subcategory, sku, startDate, endDate, current, pageSize);
        ...
        response.data.map((data, i) => {
            dataPacket.push({
                ...
                "mpnModel": data.keywordId !== null ? data.keywordId : "",
    ...
    const fetchMarket = async () => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getMarketDataShareOfSite(category);
    ...
    const fetchCountry = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getCountryDataShareOfSite(category, value);
    ...
    const fetchPartnerType = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getPartnerTypeDataShareOfSite(category, market, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchCategory = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getCategoryDataShareofSite(category, market, country, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchSubcategory = async (value) => {
        ...
        let dataPacket = [];
        let category = 'Retail';
        const response = await getSubCategoryDataShareOfSite(category, market, country, partnerType, value);
        //console.log(response.data)
        dataPacket = [...response.data];
    ...
    const fetchPartner = async (value) => {
        let dataPacket = [];
        let category = 'Retail';
        const response = await getPartnersDataShareOfSite(category, market, country, value);
        //console.log(response.data)
        dataPacket = [...response.data];
        ...
        <div>
            <div style={{ border: '0.5px solid #f0f0f0', padding: '1em', borderRadius: '5px', background: '#f0f0f0' }}>
                <Row gutter={18}>
                    ...
                    <Col xs={24} sm={24} xl={8} xxl={6} md={12} lg={8} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Tag color='processing' style={{ padding: '0.2em 1em' }}><span style={{ fontSize: '1.2em', color: '#0095d9', fontWeight: '500' }}>Share of Site Data found : </span><span style={{ fontSize: '1.2em', color: '#0095d9', fontWeight: '500' }}>{count}</span></Tag>
...
export default RetailDigitalSelf;